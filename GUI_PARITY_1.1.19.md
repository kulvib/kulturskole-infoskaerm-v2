...
## Required V2 extensions retained

Exact widget-count identity with 1.1.19 is intentionally not claimed, because
removing already-supported V2 capabilities would be a functional regression.
They are rendered inside the legacy visual grammar:

1. `Auto refresh` — canonical Display configuration field.
2. `Skift til administrator` — safe call to the existing
   `clientflow-switch-user-admin` helper, rendered as a subordinate action below
   the equal-width legacy Start/Stop primary row.

This preserves the requested legacy graphical language without deleting current
V2 operational functionality.
...
- V2 operational extensions retained: PASS
