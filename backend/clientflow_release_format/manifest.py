...
def validate_manifest(
    ...
    *,
    ...
    if data.get("artifact_type") != ARTIFACT_TYPE_RUNTIME_RELEASE:
        raise ManifestError("Manifestets artifact_type er ugyldig")
    install_modes = data.get("install_modes")
    if (
        not isinstance(install_modes, list)
        or not install_modes
        or any(not isinstance(item, str) for item in install_modes)
        or len(install_modes) != len(set(install_modes))
        or set(install_modes) - _ALLOWED_INSTALL_MODES
    ):
        raise ManifestError("Manifestets install_modes er ugyldige")
    if required_install_mode is not None:
        if required_install_mode not in _ALLOWED_INSTALL_MODES:
            raise ManifestError("Intern required_install_mode er ugyldig")
        if required_install_mode not in install_modes:
            raise ManifestError(f"Releasen understøtter ikke install mode {required_install_mode}")
