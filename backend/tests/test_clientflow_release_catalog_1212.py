...
def test_catalog_1218_promotes_exact_1317_release_identity() -> None:
    data = json.loads(CATALOG_PATH.read_text(encoding="utf-8"))

    assert data["catalog_sequence"] == 1218
    assert data["latest_stable"] == "1.3.17"
    assert data["default_install_version"] == "1.3.17"
    ...
    assert release["client_version"] == "1.3.17"
    assert release["release_sequence"] == 1218
    assert release["release_id"] == "clientflow-1.3.17-seq-1218"
    assert release["revision"] == "clientflow-1.3.17-seq-1218"
...
def test_catalog_1218_rejects_1310_and_accepts_safe_1311_in_place_source() -> None:
    load_catalog.cache_clear()
    release = resolve_release("1.3.17")
...
def test_catalog_1218_matches_current_source_and_allows_only_next_staged_identity() -> None:
    data = json.loads(CATALOG_PATH.read_text(encoding="utf-8"))
    release = data["releases"][0]
...
def test_catalog_1218_resolvers_select_1317_for_update_and_fresh_install() -> None:
    load_catalog.cache_clear()
    update = resolve_release("1.3.17")
