...
def test_head_contract_promotes_adopted_runtime_tables_out_of_opaque_preservation():
    ...
    assert 'EXPECTED_HEAD_REVISION = "20260819_49a_db_contract"' in display
    assert "EXPECTED_TABLES |= ADOPTED_RUNTIME_TABLES" in display
    assert 'REVIEWED_BASELINE_ADOPTION_HEAD = "20260819_49a_db_contract"' in runner
    assert 'REVIEWED_DATABASE_CONTRACT_REVISION = "20260819_49a_db_contract"' in runner
    assert "RECOVERABLE_LEGACY_PRESERVED_TABLES - TERMINAL_V2_TABLES - ADOPTED_RUNTIME_TABLES" in runner
