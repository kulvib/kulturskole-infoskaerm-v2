...
The normal preparation media exposes the repo-owned two-phase operator flow rather than a release selector. USB preflight verifies its own repo-owned bootstrap payload and creates **01 Klient klargøring**. Phase 1 establishes/validates office connectivity, confirms the client name, stores only root-owned non-secret factory metadata, creates **02 Aktiver ClientFlow**, removes only a NetworkManager profile that ClientFlow can prove it created itself, and offers the operator the legacy-style confirmed reboot. Release selection, enrollment authority and permanent ClientFlow account mutation do not happen in phase 1.

That account-order difference from deployed legacy 1.1.19 is intentional and security-required: V2 does not create `clientflow-kiosk` or `cfadmin` until a consuming backend claim has been accepted. The `cfadmin` password is therefore requested directly on the controlling TTY during phase 2 after the claim trust gate and is never stored in bootstrap state. No password is persisted merely to imitate the historical phase-1 prompt.
