...
When the first run stops at `pending_manual_activation`, the helper first uses the immutable staged runtime to materialize the minimum GDM/AccountsService kiosk-login baseline and then queues one controlled reboot. This happens only after durable pending state exists and does not start ClientFlow runtime services or create an active release. After backend approval, the same **Aktiver ClientFlow** helper is opened again. It reads the root-owned pending install-state, requires the existing local `clientflow-kiosk` seat0 Wayland session (foregrounding it when necessary), and invokes the immutable staged release directly through `runtime/bin/python -P -m clientflow_release activate` with the exact original release-id and approval-reference. It does not ask for or reuse the consumed CF-code. The staged canonical activation CLI performs the backend client-approval proof before runtime mutation; the stable updater remains reserved for normal post-activation polling and the explicit pre-first-activation repair path.
...
The fresh installer provisions the canonical domain/update credentials, immutable release files and rendered systemd definitions. It also materializes the stable updater host, but `clientflow-updater.timer` is explicitly kept `disabled` and `inactive`. It stops at durable `pending_manual_activation`. The outer **Aktiver ClientFlow** bootstrap then runs the staged runtime's narrow `clientflow_runtime.display_session_prepare` module, which materializes only the minimum GDM/AccountsService login baseline, and only after that succeeds queues one controlled reboot. This pre-activation preparation does not install/start Chrome, start ClientFlow runtime services, switch `/opt/clientflow/active`, enable `clientflow.target`, or weaken activation health. On a pending retry where no canonical kiosk Wayland session exists, the helper repeats the idempotent login preparation and controlled reboot instead of attempting activation. At this point the newly claimed client is still backend-pending, so its update credential is expected to remain backend-rejected and the client must not poll the update plane. A superadmin must approve that exact client through the existing canonical backend approval flow before local activation is attempted.
...
After the exact client has been approved in the backend, normal customer activation is performed by opening **Aktiver ClientFlow** again. For engineering verification, the equivalent canonical staged invocation is:
