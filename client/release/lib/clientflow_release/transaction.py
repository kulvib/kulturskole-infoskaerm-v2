...
def stage_bundle(
    ...
    *,
    ...
    if not re.fullmatch(r"[0-9a-f]{64}", str(expected_bundle_sha256 or "")):
        raise TransactionError("expected_bundle_sha256 er ugyldig")
    try:
        _bundle_size, actual_bundle_sha256 = sha256_file(bundle, max_bytes=MAX_BUNDLE_BYTES)
    except (OSError, ValueError) as exc:
        raise TransactionError(f"Releasebundlen kunne ikke hashes sikkert: {exc}") from exc
    ...
    with TransactionLock(layout):
        ...
        if release_id in installed:
            ...
            _append_history(state, "stage_idempotent", release_id=release_id)
        ...
        if final_root.exists() or final_root.is_symlink():
            ...
            state["staged_release_id"] = release_id
            _append_history(state, "stage_recovered", release_id=release_id, release_sequence=sequence)
            save_state(layout, state)
        ...
        try:
            extracted_root = extract_verified_payload(payload, temp_parent / "payload", expected_root=manifest["payload"]["root"])
        ...
        state["highest_release_sequence"] = sequence
        state["staged_release_id"] = release_id
        _append_history(state, "staged", release_id=release_id, release_sequence=sequence)
        save_state(layout, state)
...
def _activate_release(layout: Layout, state: dict[str, Any], release_id: str, approval: str) -> dict[str, Any]:
    ...
    installed[release_id]["approval_reference"] = approval
    _append_history(state, "activated", release_id=release_id, previous_release_id=previous, approval_reference=approval)
    save_state(layout, state)
    return {"status": "active", "release_id": release_id, "previous_release_id": previous}


def activate_release(release_id: str, *, approval_reference: str, layout: Layout = Layout()) -> dict[str, Any]:
    release_id = _validate_release_id(release_id)
    approval = _validate_approval(approval_reference)
    with TransactionLock(layout):
        state = load_state(layout)
        return _activate_release(layout, state, release_id, approval)
...
def rollback_release(
    *,
    ...
    with TransactionLock(layout):
        ...
        state = load_state(layout)
        _append_history(state, "manual_rollback", from_release_id=current, to_release_id=target, reason=reason, approval_reference=approval)
        save_state(layout, state)
