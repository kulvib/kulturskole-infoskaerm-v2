...
Hos kunden lander maskinen direkte i `clientflow-kiosk`, og **02 Aktiver ClientFlow** åbnes én gang uden sudo-passwordprompt. Rækkefølgen er kundenetværk, lokation og derefter den formatterede `CF-____-____-____` kode. Den normale 02-dialog skjuler den canonical installers afsluttende machine-readable JSON-resultat; direkte engineering-kald til installer-CLI beholder JSON-outputtet uændret. Helperen henter internt den durable exact release-binding og signerede fresh-install authorization, downloader den exact approved bundle med synlig fremdrift, verificerer SHA-256/størrelse samt embedded installer og sender one-time authorities til canonical installer over stdin. Den canonical installer accepterer kun de allerede eksisterende menneskelige konti, når den root-ejede schema-2 factory-state er markeret `handoff_ready` og konto-kontrakten kan genvalideres. Kunden bliver aldrig bedt om at vælge eller kende `cfadmin`-password.

Når `pending_manual_activation` er durable, fjerner helperen straks den midlertidige activation-sudoers-ret, trust-helperen og **02**-ikonet, genmaterialiserer den minimale canonical GDM/AccountsService kiosk-login-baseline fra den staged runtime, installerer den midlertidige status-only `clientflow-preactivation-gui.service`, enable'r `clientflow-first-activation.service` og tilbyder den anden brugerbekræftede genstart. Efter reboot viser den staged releases eksisterende legacy-layout GUI `Registreret – afventer godkendelse`; Start/Stop kiosk er disabled, der findes ingen administrator-switch handling i GUI'en, og ingen ClientFlow runtime-domæner startes. Når en superadmin efterfølgende godkender præcis klienten, kalder waiteren automatisk den immutable staged canonical `clientflow_release activate`; den re-beviser backend approval før lokal runtime-mutation. `clientflow.target` konflikter den midlertidige GUI-service væk, og den normale release-owned GUI overtager som del af Display-runtime. Kunden skal **ikke** åbne Aktiver ClientFlow igen efter godkendelsen.
...
15. Efter durable `pending_manual_activation` materialiserer den ydre **02 Aktiver ClientFlow**-helper kun den minimale GDM/AccountsService login-baseline fra den staged runtime, installerer den midlertidige status-only pre-activation GUI-service, enable'r den snævre first-activation waiter og tilbyder derefter én brugerbekræftet reboot. Rebootkommandoen bruger `systemctl --no-block --check-inhibitors=no reboot`, timeout 10 sekunder og aldrig `--force`. Rebootet etablerer kiosk-Wayland-sessionen og viser den staged releases eksisterende lokale GUI i `Afventer godkendelse`-mode; det aktiverer ikke releasen, starter ikke browseren og starter ikke noget ClientFlow runtime-domæne.

Der sker **ingen runtime-aktivering før backend-godkendelse**, ingen åbning af Terminal eller Remote Desktop, ingen start af en livestream og ingen updater-polling i `pending_manual_activation`. Den eneste synlige ClientFlow-proces er den separate, unprivileged status-GUI, som læser offentlig klientidentitet og lokal netværksstatus og holder alle muterende kiosk-knapper disabled. Waiteren må kun forsøge den canonical staged activation, og denne gate fejler lukket, indtil backend approval-proof er gyldigt.
