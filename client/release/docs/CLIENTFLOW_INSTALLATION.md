...
Den normale brugerrejse følger legacy 1.1.19's to synlige faser, men med V2's fail-closed release- og enrollment-grænser under overfladen. Det repo-ejede USB-bootstrap opretter først **01 Klient klargøring**. Kontorfasen etablerer netværk, bekræfter klientnavn, gemmer kun root-ejet ikke-hemmelig factory-state, opretter **02 Aktiver ClientFlow** og tilbyder en brugerbekræftet genstart. USB-mediet indeholder ingen `latest`/`stable` release-authority.

Hos kunden åbnes **02 Aktiver ClientFlow** én gang. Rækkefølgen er kundenetværk, lokation og derefter den formatterede `CF-____-____-____` kode. Helperen henter internt den durable exact release-binding og signerede fresh-install authorization, downloader den exact approved bundle, verificerer SHA-256/størrelse samt embedded installer og sender one-time authorities til canonical installer over stdin. Efter en gyldig consuming claim sættes `cfadmin`-password direkte på TTY; passwordet persisteres aldrig. Denne placering af passwordprompten er den bevidste V2-sikkerhedsafvigelse fra legacy: permanente menneskelige konti oprettes ikke før backend trust gate.

Når `pending_manual_activation` er durable, materialiserer helperen kun den minimale GDM/AccountsService kiosk-login-baseline, enable'r `clientflow-first-activation.service`, fjerner installationsikonet og tilbyder den anden brugerbekræftede genstart. Klienten forbliver fail-closed og backend-`pending`. Når en superadmin efterfølgende godkender præcis klienten, kalder waiteren automatisk den immutable staged canonical `clientflow_release activate`; den re-beviser backend approval før lokal runtime-mutation. Kunden skal **ikke** åbne Aktiver ClientFlow igen efter godkendelsen.
