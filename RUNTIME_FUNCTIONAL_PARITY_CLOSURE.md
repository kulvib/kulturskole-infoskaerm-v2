...
- frontend source-only Node contracts: 95/95 passed; the 3 remaining frontend tests require missing local `vite`/`react` dependencies and are canonical-CI gated.
- legacy capability validate: 13/13 capabilities, 177/177 inventory, status OK.
- Python source syntax: 254 files parsed/compiled without bytecode.
- relevant Bash syntax: passed.
- DB-backed operational HTTP integration: locally skipped because `sqlmodel/passlib` are absent; the new OS-update reboot-resume test is included for canonical CI.
...
## Final candidate verification — 2026-09-07

Canonical candidate basis: fresh uploaded v2 main repository.  This closure batch is limited to documented legacy-1.1.19 functional-parity/runtime blockers and does not change the Frozen Livestream, Terminal, or Remote Desktop implementations.

Final local gates after cumulative reconstruction:

- `python -m pytest -q scripts/tests`: 184 passed.
- relevant backend source/runtime tests not requiring SQLModel: 45 passed.
- frontend readiness UI contract: 7 passed.
- legacy 1.1.19 capability gate: 13/13 capabilities, 177/177 inventory, status `ok`.
- Python compileall: pass.
- changed shell helpers syntax: pass.
- Frozen path SHA comparison against fresh main: 73 checked, 0 drift.

Environment limitation: DB-backed backend integration tests importing `sqlmodel` cannot be collected in the local review container because `sqlmodel` is not installed there. They are not counted as passing and remain mandatory in canonical CI.

Product timing policy in this candidate: all visible Display/browser countdowns are 10 seconds. Calendar polling/evaluation remains 15s/30s with 90s boot grace, 15s wake-to-reboot delay, and 300s reboot cooldown. Shared-domain status reports are 15s with the canonical 120s offline boundary.
